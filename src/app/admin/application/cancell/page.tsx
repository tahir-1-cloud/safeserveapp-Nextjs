...
import { getCancelApplication } from '@/services/application';
import CustomLoader from '@/components/CustomerLoader';

export default function CancelledLeavePage() {
  const [applications, setApplications] = useState<LeaveApplication[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [pageSize, setPageSize] = useState(10);

  // Fetch cancelled applications once
  useEffect(() => {
    getCancelApplication()
      .then(data => setApplications(data))
      .catch(err => {
        console.error('Error fetching cancelled applications:', err);
      })
  ...
  const columns: ColumnsType<LeaveApplication> = useMemo(() => [
    ...
    { title: 'End Time', dataIndex: 'endTime', key: 'endTime', width: 95 },
    {
      ...
      key: 'status',
      width: 100,
      render: () => <span className="text-red-600 font-semibold">Cancelled</span>,
    },
  ], []);
