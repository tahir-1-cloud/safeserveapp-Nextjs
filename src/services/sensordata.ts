...
import { SensorData,TempChecksModel,ManulLimitModel } from '@/types/sensordto';
...
export const getLiveSensorDataWithAssetName =
  ...
  export const getTempManualLimit = async (): Promise<ManulLimitModel[]> => {
    try{
        const response = await axiosInstance.get<ManulLimitModel[]>(`/TemperatureUnit/GetManualLimits`);
        return response.data;
    } 
    catch (error: unknown) {
        if (axios.isAxiosError(error)) {
